...
from STPLS3D_test import ModelTester
...
class STPLS3D:
    def __init__(self, dataFolder, test_area_idx):
        self.name = 'STPLS3D'
    ...
    def get_batch_gen(self, split):
        ...
        def spatially_regular_gen():
            ...
            for i in range(num_per_epoch):
                ### file = open(r'D:\syntheticVSreal_experiements\RandLA-Net\data\temp%d.txt'%i,'w')
                ...
                # for i,point in enumerate(queried_pc_xyz):
                #     file.write("%f,%f,%f,%f,%f,%f,%d\n"%(point[0],point[1],point[2],
                #                                        queried_pc_colors[i][0],queried_pc_colors[i][1],queried_pc_colors[i][2],
                #                                        queried_pc_labels[i]))
                if True:
    ...
    def load_evaluation_points(file_path):
        ...
        return np.vstack((data['x'], data['y'], data['z'])).T,\
               np.vstack((data['red'], data['green'], data['blue'])).T

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    # parser.add_argument('--gpu', type=int, default=0, help='the number of GPUs to use [default: 0]')
    parser.add_argument('--data_folder', type=str, default=r'E:\STPLS3D\STPLS3D_prepared\RealWorldData', help='The directory that contains all data')
    parser.add_argument('--test_area', type=str, default='USC_GT', help='Which area to use for test, option: 1-6 [default: 5]')
    ...
    if Mode == 'train':
        ...
        cfg.saving = False
        model = Network(dataset, cfg)
        if os.path.isfile(FLAGS.model_path+'.meta'):
            chosen_snap = FLAGS.model_path
            tester = ModelTester(model, dataset, restore_snap=chosen_snap)
            IoUs,m_IoU,acc = tester.test(model, dataset)

        elif os.path.isdir(FLAGS.model_path):
            checkpointList = [f for f in os.listdir(FLAGS.model_path) if f.endswith('.meta')]
            IoUs_list = []
            m_IoU_list = []
            acc_list = []
            ...
            print ("Best mIOU: %s" % m_IoU_list[best_mIOUindx])
            print ("Best IOUs: %s" % IoUs_list[best_mIOUindx])
            print ("Best acc: %s" % acc_list[best_mIOUindx])
        else:
            chosen_snapshot = -1
            logs = np.sort([os.path.join('results',f) for f in os.listdir('results') if f.startswith('Log')])
            chosen_folder = logs[-1]
            snap_path = chosen_folder +'/'+ 'snapshots'
            snap_steps = [int(f[:-5].split('-')[-1]) for f in os.listdir(snap_path) if f[-5:] == '.meta']
            chosen_step = np.sort(snap_steps)[-1]
            chosen_snap = os.path.join(snap_path,'snap-{:d}'.format(chosen_step))
            ### Model used
            # chosen_snap = r'D:\syntheticVSreal_experiements\RandLA-Net\results\Log_2021-09-21_21-05-21\snapshots\snap-45501'
