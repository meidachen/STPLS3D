                ...
                if last_min + 1 < new_min:
                    ...
                    if int(np.ceil(new_min)) % 1 == 0:
                        ...
                        for i_test in range(num_val):
                            ...
                            log_out(name, self.Log_file)

                            points,colors = dataset.load_evaluation_points(os.path.join(dataset.path,'original_ply',name))


                            write_ply(join(test_path, 'val_preds', name), [points, colors, preds, labels], ['x', 'y', 'z', 'red', 'green', 'blue', 'pred', 'label'])
